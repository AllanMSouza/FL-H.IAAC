...
np.random.seed(0)
torch.manual_seed(0)
class FedDistillClientTorch(ClientBaseTorch):
				...
				for step in range(max_local_steps):
					...
					for i, (x, y) in enumerate(self.trainloader):
						...
						if self.global_logits != None:
							logit_new = copy.deepcopy(output.detach().numpy())
							for i, yy in enumerate(y):
								y_c = yy.item()
								if y_c < len(self.global_logits) and type(self.global_logits[y_c]) != type([]):
									logit_new[i, :] = self.global_logits[y_c]
							loss += self.loss_mse(logit_new, output)

						for i, yy in enumerate(y):
							y_c = yy.item()
							logits[y_c].append(output[i, :].detach().numpy())
	...
	def set_parameters_to_model(self, parameters=[]):
		filename = """./feddistill_saved_weights/{}/{}/model.pth""".format(self.model_name, self.cid, self.cid)
	...
	def save_parameters(self):
		...
		# 			exist_ok=True)
		try:
			filename = """./feddistill_saved_weights/{}/{}/model.pth""".format(self.model_name, self.cid)
	...
	def set_logits(self, logits):
		self.global_logits = copy.deepcopy(logits)
	...
	def agg_func(self, logits):
		...
		try:
			...
			return[np.array(logits[label]) for label in logits]

		except Exception as e:
			print("agg_func client")
			print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e)
	...
	# 			numpy_protos = copy.deepcopy(self.global_logits)
	# 		else:
