...
torch.manual_seed(0)
class FedDistillBaseServer(FedAvgBaseServer):
    ...
    def __init__(self,
        ...
        self.global_logits = [np.array([np.nan]) for i in range(self.n_classes)]
    ...
    def create_folder(self):
        ...
        directory = """feddistill_saved_weights/{}/""".format(self.model_name)
        if Path(directory).exists():
            shutil.rmtree(directory)
        for i in range(self.num_clients):
            Path("""feddistill_saved_weights/{}/{}/""".format(self.model_name, i)).mkdir(parents=True, exist_ok=True)
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        for _, fit_res in results:
            ...
            if self.aggregation_method not in ['None', 'POC', 'FedLTA'] or int(server_round) <= 1:
                weights_results.append(fl.common.parameters_to_ndarrays(fit_res.parameters))
        ...
        parameters_aggregated = fl.common.ndarrays_to_parameters(self.logit_aggregation(weights_results, server_round))
    ...
    def logit_aggregation(self, local_logits_list, server_round):
        agg_logits_label = defaultdict(list)
        for local_logits in local_logits_list:
            for i in range(len(local_logits)):
                agg_logits_label[str(i)].append(local_logits[i])

        for [label, logit_list] in agg_logits_label.items():
            if len(logit_list) > 1:
                logit = 0 * logit_list[0]
                for i in logit_list:
                    logit += i
                agg_logits_label[label] = logit / len(logit_list)
            else:
                agg_logits_label[label] = logit_list[0]

        return [np.array(agg_logits_label[key]) for key in agg_logits_label]

    def _aggregate_proto(self, results, server_round):
        """Compute weighted average."""

        for i in range(len(self.global_logits)):
            w = self.global_logits[i]
        ...
        for i in range(self.n_classes):
            ...
            if (np.isnan(new_proto).any() or np.sum(new_proto) == 0) and (not np.isnan(self.global_logits[i]).any() and np.sum(self.global_logits[i]) > 0):
                weighted_weights[i] = copy.deepcopy(self.global_logits[i])
        ...
        for i in range(self.n_classes):
            ...
            if np.isnan(proto).any() or np.sum(proto) == 0:
                ...
                self.global_logits[i] = copy.deepcopy(proto)
